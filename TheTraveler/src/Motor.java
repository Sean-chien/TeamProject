...
public class Motor {
    ...
    public Motor(Jogo j) {
        ...
        JFrame container = new JFrame(jogo.getTitle());
        JPanel panel = (JPanel) container.getContentPane();
        panel.setPreferredSize(new Dimension(jogo.getWidth(), jogo.getHeight()));
        panel.setLayout(null);
        GraphicsEnvironment ge = GraphicsEnvironment.getLocalGraphicsEnvironment();
        GraphicsDevice[] gs = ge.getScreenDevices();
        Rectangle bounds = gs[gs.length - 1].getDefaultConfiguration().getBounds();
        container.setResizable(false);
        container.setBounds(bounds.x + (bounds.width - jogo.getWidth()) / 2,
                bounds.y + (bounds.height - jogo.getHeight()) / 2,
                jogo.getWidth(), jogo.getHeight());
        canvas.setBounds(0, 0, jogo.getWidth(), jogo.getHeight());
        panel.add(canvas);
        ...
        Timer t = new Timer(5, new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                if (t1 > t0) {
                    ...
                    g.fillRect(0, 0, jogo.getWidth(),
                            jogo.getHeight());
                    jogo.desenhar(new Scene(g));
                    strategy.show();
                }
            }
        });
